...
<!--
Vincent Driessen 這篇文章一發出來就在開發者社群引起廣大迴響，因為它完美地用文字描述了很多團隊當時在痛苦摸索的東西。有趣的是，他自己在 2020 年於文章底部加了說明，表示這個模型適合有版本號的套件，但對持續交付的 web app 可能 GitHub Flow 更適合。接下來看適用條件。
-->
...
- 需要同時維護多個版本的專案
- 發布週期較長（週發布或月發布）
- 需要嚴格 QA 流程，發布前有專屬測試期

<div style="margin-top: 1.5rem; padding: 1rem 1.5rem; background: #fef3c7; border-left: 4px solid #d97706; border-radius: 4px;">

**作者補充（2020）：** Vincent Driessen 在文章底部加了說明——此模型適合有版本號的套件，對持續交付的 web app 可能 GitHub Flow 更適合。選擇最適合團隊的工作流程才是重點。

</div>

<!--
能在十年後坦然補充，說明這個作者很有誠信。接下來看五種分支各自的角色。
-->
...
<!--
這個合併規則是 Git Flow 的骨幹。feature 不能直接進 main，確保 main 的程式碼永遠是穩定的。接下來看關鍵規則說明。
-->
...
**為什麼 release 和 hotfix 要同時合到兩邊？**

| 動作 | 合到 main | 合回 develop |
| --- | --- | --- |
| release 完成 | 正式上線 | 確保下個版本包含 release 期間的 bug 修正 |
| hotfix 完成 | 緊急上線 | 確保下個版本不會再出現同樣的 bug |

<!--
release 和 hotfix 都要同時合到 main 和 develop，這點很重要：如果只合到 main 而沒有合回 develop，下個版本就會漏掉這次的修正，等於白修了。這個「雙向合併」的機制確保修復不會在版本之間失蹤。
-->
...
<!--
Git Flow 並不適合所有場景。接下來看不適合的情境和替代策略。
-->

---

# Git Flow 的適用時機：不適合的情境
...
<div style="margin-top: 1.5rem; padding: 1rem 1.2rem; background: #fef3c7; border-left: 4px solid #d97706; border-radius: 4px;">
...
<!--
如果我們在做一個 web app，每天部署好幾次，用 Git Flow 的 release 流程會讓大家覺得很麻煩。GitHub Flow 這種更輕量的策略反而更適合。不是說 Git Flow 不好，是要選適合自己團隊的工具。接下來我們來看怎麼實際操作 Git Flow。
-->
...
git-flow 工具不是必要的，你完全可以用原生 git 指令手動執行所有步驟。但 git-flow 把這些操作包裝起來，減少出錯機會。
...
<!--
git flow init 會問你各種分支的命名，預設值就是 Git Flow 的標準命名，一般直接按 Enter 就好。初始化之後，它會自動建立 develop 分支並切換過去。接下來看新功能開發流程。
-->
...
<!--
git flow feature start 從 develop 建立分支並切換過去，之後正常開發即可。接下來看完成功能的指令。
-->
...
**`--no-ff` 的作用：** 強制產生 merge commit，讓歷史圖形中可以清楚看到 feature 分支的起點和終點。不加的話，commit 直接「接」在 develop 上，看不出屬於哪個功能。

</div>

<!--
--no-ff 在 Git Flow 中很重要。如果用 fast-forward 合併，feature 分支的 commit 會直接「接」在 develop 上，你無法從 git log 看出這些 commit 屬於哪個功能。接下來看純 git 指令的版本。
-->

---

# 新功能開發流程（純 Git 指令版）：開始
...
<!--
純 Git 指令版本步驟多一點，但讓我們更清楚每一步在做什麼。接下來看完成功能的部分。
-->

---

# 新功能開發流程（純 Git 指令版）：完成
...
實際工作中，通常用 GitHub PR 來做 feature → develop 的合併，因為 PR 提供 code review 機制。git-flow 工具的優勢主要在小團隊或不用 PR 的場景。

</div>

<!--
在實際工作中，通常會用 GitHub 的 PR 功能來做 feature 到 develop 的合併，而不是在 command line 直接 merge，因為 PR 提供了 code review 的機制。接下來看版本發布流程。
-->
...
**release 分支的鐵則：只修不加。** 只能做小 bug 修正、版本號更新、CHANGELOG，不能加任何新功能。

</div>

<!--
release 分支的重點是「只修不加」——在這個分支上只能做小 bug 修正、版本號更新、文件更新，不能加任何新功能。接下來看完成發布的指令。
-->
...
`release finish` 是複合操作：合到 main、打 tag、合回 develop、刪除 release 分支。用純 git 指令要手動四步，這裡 git-flow 工具優勢最明顯。

</div>

<!--
git flow release finish 會同時合到 main、打 tag、合回 develop，是個複合操作，用純 git 指令要手動做好幾步，所以這裡 git-flow 工具的優勢特別明顯。接下來看緊急修復流程。
-->
...
**關鍵：** hotfix 從 `main` 建立，不是從 `develop`，確保只包含線上版本的程式碼，不夾帶 develop 上未完成的功能。

</div>

<!--
hotfix 流程是 Git Flow 設計中最精彩的部分。它完美解決了「線上有 bug 但 develop 上有未完成功能」的困境。接下來看完成 hotfix 的指令。
-->
...
`hotfix finish` 同時合到 main 和 develop，確保這個修復在下個版本也有，不會再出現同樣的 bug。

</div>

<!--
hotfix 從 main 建立，基礎是乾淨的線上版本，修完 bug 合到 main 就立刻可以上線，不需要擔心 develop 上的東西會跟著出去。同時合到 develop，確保這個修復在下個版本也有，不會再出現同樣的 bug。接下來看完整的流程圖。
-->
...
<!--
main 和 develop 是兩條平行的長期分支，其他分支都是短暫的，完成任務就消失。接下來看 main 上每個 commit 代表什麼意義。
-->
...
看 main 的 `git log`，就能清楚知道「1.0.0 是什麼時候發布的，1.0.1 是修了什麼緊急 bug」。這種可追溯性對維護和審計非常有價值。

</div>

<!--
這張流程圖展示了 Git Flow 的全貌。main 上的每個合併 commit 都對應一個版本號，代表一個正式發布。接下來我們來比較三種工作流程。
-->
...
<!--
沒有哪個工作流程是完美的，關鍵是要跟你的團隊、你的產品類型、你的部署頻率相匹配。接下來看選擇建議。
-->

---

# Git Flow vs GitHub Flow vs Trunk-based：選擇建議

| 工作流程 | 最適合 |
| --- | --- |
| **Git Flow** | 有版本號的產品、週期性發布、需要嚴格 QA |
| **GitHub Flow** | Web 應用、隨時部署、小型團隊快速迭代 |
| **Trunk-based** | 大型持續交付、有強大自動化測試基礎 |

<div style="margin-top: 1.5rem; padding: 1rem 1.5rem; background: #fff8f0; border-left: 4px solid #d97706; border-radius: 4px;">

**建議：** 先了解 Git Flow 打好基礎，再根據團隊的實際需求選擇或客製化最適合的工作流程。很多大公司用的是介於這三者之間的客製化版本。

</div>

<!--
GitHub Flow 最簡單，門檻低，適合快速迭代的 web 應用。Git Flow 最完整，適合需要嚴格版本管理的產品。Trunk-based 最激進，需要強大的自動化測試和 feature flags 基礎設施作為支撐。
-->
...
<!--
這一章看了 Git Flow 的五種分支角色，以及如何根據團隊規模選擇工作流程。如果對 Git Flow 的導入或分支策略有任何疑問，歡迎提出來討論！
-->
