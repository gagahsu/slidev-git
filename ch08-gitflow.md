...
<!--
歡迎來到第八章「使用 Git Flow」。前一章我們學了 GitHub 上的協作工具，但光有工具還不夠，多人團隊還需要有「規則」——什麼時候該開新分支？功能做完要合併到哪裡？如何準備一個正式發布版本？Git Flow 就是一套回答這些問題的分支管理規範，由 Vincent Driessen 在 2010 年提出，至今仍是業界廣泛採用的標準之一。
-->
...
<!--
這章比較精煉，兩個大主題。第一個是概念：為什麼需要 Git Flow，它解決了什麼問題，五種分支各自的角色是什麼。第二個是實作：怎麼用 git-flow 工具執行完整的開發、發布、緊急修復流程，以及和其他工作流程的比較。
-->
...
<!--
先從問題出發：如果沒有 Git Flow，沒有任何規範，團隊協作會有什麼問題？
-->
...
<!--
這些情境都是真實存在的，不只是假設。特別是「線上緊急 bug 但不能上版」這個問題，在沒有分支規範的團隊裡是噩夢。你需要把 main 上那些未完成的功能先 revert 掉，修完 bug，再把 revert 的東西還原，稍不注意就出錯。Git Flow 的設計就是為了從根本上解決這類問題。
-->
...
<!--
Vincent Driessen 2010 年的那篇文章一發出來就在開發者社群引起廣大迴響，因為它完美地用文字描述了很多團隊當時在痛苦摸索的東西。他自己後來（2020 年）在文章底部加了一個說明，表示這個模型很適合有版本號的套件，但對持續交付的 web app 可能 GitHub Flow 更適合。這個誠實的補充讓我更尊敬這個作者。
-->
...
<!--
五種分支的設計非常有道理。main 是「對外」的門面，永遠代表已上線的版本，不接受直接 push，只接受 release 和 hotfix 的合併。develop 是「對內」的整合點，所有功能做完都先合進 develop，在這裡測試和整合，確認沒問題再整理成 release 準備上線。feature 分支是最多的，每個功能獨立一個，互不干擾。release 和 hotfix 是有時效性的，完成任務就要刪掉。
-->
...
<!--
這個合併規則是 Git Flow 的骨幹。feature 不能直接進 main，這個限制確保了 main 的純潔性。release 分支同時合到 main 和 develop，是因為在 release 分支上可能做了一些小 bug 修正，這些修正也要同步回 develop，否則下個版本又要修一次。hotfix 同樣的道理，既要合到 main（修好線上 bug），也要合到 develop（避免下個版本又有同樣的 bug）。
-->
...
<!--
這是很多人沒想到的一點：Git Flow 並不適合所有場景。如果你在做一個 web app，每天部署好幾次，用 Git Flow 的 release 流程會讓你覺得超麻煩。GitHub Flow 這種更輕量的策略反而更適合：只有 main 和 feature 兩種分支，feature 完成後開 PR 審查，merge 後直接部署。Trunk-based Development 更激進，直接在 main 上開發，用 feature flags 控制功能的開關，這是 Google 內部的主要工作方式。
-->
...
<!--
git-flow 工具不是必要的，你完全可以用原生的 git 指令手動執行 Git Flow 的所有步驟。但 git-flow 把這些操作包裝起來，減少出錯的機會，也讓流程更容易記憶。git flow init 會問你各種分支的命名，預設值就是 Git Flow 的標準命名，一般直接按 Enter 就好。初始化之後，它會自動建立 develop 分支（如果不存在的話）並切換過去。
-->
...
<!--
--no-ff 是 Git Flow 中很重要的選項。如果用 fast-forward 合併，feature 分支的 commit 會直接「接」在 develop 上，看起來就像是直接在 develop 上開發的，你無法從 git log 上看出這些 commit 是屬於哪個功能的。--no-ff 強制產生一個 merge commit，讓歷史圖形中可以清楚看到 feature 分支的起點和終點。這對 code review 和追蹤功能歷史很有幫助。
-->
...
<!--
純 Git 指令版本雖然步驟多一點，但讓你更清楚每一步在做什麼。在實際工作中，通常會用 GitHub/GitLab 的 PR 功能來做 feature 到 develop 的合併，而不是在 command line 直接 merge，因為 PR 提供了 code review 的機制。git-flow 工具主要的優點是在不用 PR 的場景（例如小團隊或個人專案）讓操作更便利。
-->
...
<!--
release 分支的重點是「只修不加」——在這個分支上只能做小 bug 修正、版本號更新、文件更新，不能加任何新功能。新功能要排到下一個版本的 develop 去。這個規則確保了 release 分支的穩定性。git flow release finish 會同時做三件事：合到 main、打 tag、合回 develop，是個複合操作，用純 git 指令要手動做好幾步，所以這裡 git-flow 工具的優勢特別明顯。
-->
...
<!--
hotfix 流程是 Git Flow 設計中我覺得最精彩的部分。它完美解決了「線上有 bug 但 develop 上有未完成功能」的困境。hotfix 從 main 建立，所以它的基礎是乾淨的線上版本，修完 bug 合到 main 就立刻可以上線，不需要擔心 develop 上的東西。同時合到 develop，確保這個修復在下個版本也有。這個流程讓緊急修復有一個清晰、可重複的路徑。
-->
...
<!--
這張流程圖展示了 Git Flow 的全貌。main 和 develop 是兩條平行的長期分支，其他分支都是短暫的，完成任務就消失。main 上的每個合併 commit 都對應一個版本號（tag），代表一個正式發布。看到 main 的 git log，你就能清楚知道「1.0.0 是什麼時候發布的，1.0.1 是修了什麼緊急 bug」。這種可追溯性對於維護和審計非常有價值。
-->
...
<!--
這個比較表是整章最重要的一個總結。沒有哪個工作流程是完美的，關鍵是和你的團隊、你的產品類型、你的部署頻率相匹配。GitHub Flow 最簡單，門檻低，適合快速迭代的 web 應用。Git Flow 最完整，適合需要嚴格版本管理的產品。Trunk-based 最激進，需要強大的自動化測試和 feature flags 基礎設施作為支撐，否則會很混亂。很多大公司其實用的是介於這三者之間的客製化版本。
-->
...
<!--
這些是導入 Git Flow 時最常踩到的坑。分支保護規則是非常重要的技術保障，可以防止人在疲勞或緊張的情況下不小心直接 push 到 main。feature 分支太長壽是另一個大問題，有些人開了個 feature 分支，然後去忙其他事，兩個月後回來發現 develop 已經差了幾十個 commit，解衝突就花了一整天。如果功能很大，考慮把它拆成多個小的 feature，逐步合進 develop。
-->
...
<!--
第八章到這裡結束，也是整個 Git 版本控制課程的最後一章。從第一章的版本控制概念，到最後的 Git Flow 工作策略，我們走過了 Git 從本機使用到多人協作的完整旅程。希望這堂課能讓大家在工作中用 Git 更有信心，遇到狀況不再慌亂，也能根據團隊的需求選擇最合適的協作方式。祝大家 commit 順利，merge 無衝突！
-->
