...
<!--
這三個指令是 Git 日常使用的核心，幾乎每次 commit 都會用到。git status 是我們最好的朋友，隨時確認現在的狀態；git add 把改動「放進暫存區」；git commit 才是真正的「提交」。commit 訊息請寫清楚，未來的你或同事看歷史記錄時會感謝現在的你。避免寫「改了一些東西」、「測試」這類沒有意義的訊息。接下來看一個很多新手容易踩的坑。
-->
