...
<!--
歡迎來到第三章：使用分支。分支是 Git 最強大的特性之一，也是讓 Git 跟其他版本控制系統拉開差距的關鍵功能。這一章我們會從「為什麼需要分支」開始，一路介紹怎麼建立、切換、合併，以及遇到問題時怎麼解決。
-->
...
<!--
這是本章的大綱，共有十一個主題。前三個是核心操作，中間幾個是常見情境與解法，最後兩個冷知識會幫助你理解 Git 分支的底層設計，讓你遇到問題時不再只是亂試指令。
-->
...
<!--
我們從最根本的問題開始：分支到底解決了什麼問題？為什麼幾乎所有的開發流程都強調要用分支？
-->
...
<!--
想像一下，你正在寫一個新功能，程式碼寫到一半，突然客戶打電話來說有個嚴重的 bug 要馬上修。這時候你怎麼辦？你現有的改動還不能提交，但你又必須切換到修 bug 的狀態。沒有分支的話，這個情境就很難處理。
-->
...
<!--
分支讓你可以同時在多條時間線上工作。功能 A 在它自己的線上，功能 B 在另一條線上，hotfix 又是另一條，它們互相不干擾。這就是為什麼現代開發流程，不管是 Git Flow、GitHub Flow 還是 trunk-based development，都以分支作為基礎。
-->
...
<!--
這張投影片說明分支對多人協作的價值。重點不是說「用分支就沒有衝突」，而是說衝突變得可預測、可管理。Alice 和 Bob 各自在自己的分支工作，不會彼此打擾，合併的時候再一起解決差異，這樣的流程遠比大家搶著提交同一份程式碼要清晰得多。
-->
...
<!--
了解了為什麼需要分支之後，我們來學怎麼操作。Git 有幾個指令可以管理分支，我們一一來看。
-->
...
<!--
git branch 是最基礎的分支管理指令。加上 -v 參數特別好用，可以一眼看出每個分支現在指向哪個 commit。建立分支之後，HEAD 還是停在原本的位置，需要另外切換過去。
-->
...
<!--
Git 2.23 版引入了 git switch 和 git restore，把 git checkout 的功能拆分得更清楚。switch 專門用來切換分支，restore 專門用來還原檔案。雖然 checkout 還是能用，但建議新手優先學 switch，語意比較直觀。建立並切換是最常見的操作，每次開新功能都會用到。
-->
...
<!--
分支的生命週期通常是：建立 → 開發 → 合併 → 刪除。刪除已合併的分支是良好習慣，可以避免分支列表越來越長，難以管理。小寫 -d 有保護機制，如果分支還沒合併會拒絕刪除；大寫 -D 則是強制刪除，後面的狀況題會說明萬一刪錯了怎麼救回來。
-->
...
<!--
分支開發完成之後，要把改動合併回去。Git 的合併有兩種主要模式，理解它們的差異，對後面理解「小耳朵」的問題很重要。
-->
...
<!--
合併的方向很容易搞混。記住一個原則：你現在在哪個分支，就是把別人合進來。所以要把 feature 合進 main，就先切換到 main，再 merge feature。git log --oneline --graph 是確認合併結果的好朋友，一定要養成習慣。
-->
...
<!--
Fast-forward 是最簡單的合併情境：main 自從你開分支之後就沒動過，所以 Git 只需要把 main 指標往前移動到 feature 的最新 commit，不需要額外的合併動作。結果是歷史看起來很乾淨，但也失去了「曾經有個分支」的資訊。
-->
...
<!--
3-way merge 的「3」指的是三個節點：兩個分支各自的最新 commit，以及它們的共同祖先。Git 把這三個版本做比較，聰明地合併差異。這就是為什麼叫 3-way：不是只看兩個版本的差，而是要知道「從哪裡開始分開的」才能正確合併。
-->
...
<!--
很多人剛開始學 Git 圖形化工具時，會疑惑為什麼有時候 log 圖上有分叉，有時候沒有。這就是「小耳朵」問題。
-->
...
<!--
「小耳朵」是口語說法，正式說法是「合併 commit 的分叉線」。有沒有小耳朵，取決於合併時有沒有產生合併 commit。Fast-forward 不產生，所以沒有小耳朵；3-way merge 會產生，所以有小耳朵。
-->
...
<!--
要不要加 --no-ff 是一個團隊風格的選擇。有些人喜歡線性歷史（用 rebase 或 fast-forward），有些人喜歡保留分支痕跡（用 --no-ff）。沒有絕對的對錯，但整個團隊要統一規則，否則 log 圖會很混亂。
-->
...
<!--
開發久了，分支列表可能會越來越長。什麼分支可以刪、什麼不能刪，是很多人的疑問。
-->
...
<!--
git branch --merged 非常好用，定期執行可以保持分支列表乾淨。已合併的分支即使刪掉，commit 還在，只是少了一個名字而已，所以可以安全刪除。未合併的分支刪掉就是真的遺失那些 commit，需要謹慎。
-->
...
<!--
遠端分支的清理常常被忽略。當同事在 GitHub 或 GitLab 上刪除了某個分支，你的本地端還是會有一個 origin/feature-xxx 的追蹤引用。git fetch --prune 可以把這些失效的引用清掉，讓 git branch -a 的輸出更乾淨。
-->
...
<!--
這是很多人都遇過的狀況：手滑打了 git branch -D，或者搞錯分支名稱，把還沒合併的分支給刪掉了。別慌，Git 有辦法救回來。
-->
...
<!--
Git 的分支只是一個指標，刪除分支只是刪掉那個指標，commit 本身還留在 Git 的物件資料庫裡。Git 有一個垃圾回收機制，只有在沒有任何引用的 commit 「孤立」超過一段時間後，才會真正被清除。所以刪除後趕快找，通常都救得回來。
-->
...
<!--
git reflog 是 Git 裡最重要的救援指令之一。只要你的操作在本地端發生，幾乎都可以用 reflog 找回來。它記錄了每一次 HEAD 移動的快照，所以即使分支指標消失了，commit hash 還留在 reflog 裡。找到 hash 之後，用 git branch 重新建立指標就好了。
-->
...
<!--
除了 merge 之外，還有另一種整合分支的方式叫做 rebase。它的哲學和 merge 不同，各有適合的使用場景。
-->
...
<!--
Rebase 的字面意思是「重新設定基礎」。它把你的 commit 從原來的起點拔起來，重新接在新的基礎上。結果是 C 變成了 C'，D 變成了 D'，內容相同，但 hash 不同，因為它們的父節點已經改變了。
-->
...
<!--
Rebase 最大的優點是歷史線性，看起來乾淨；最大的風險是它修改了 commit hash，如果已經 push 到遠端，別人也拉了那些 commit，你 rebase 之後就會造成歷史分歧，非常麻煩。黃金原則：只對還沒有推上去的本地 commit 做 rebase。
-->
...
<!--
Rebase 中途發生衝突時，要逐個 commit 解決，每解完一個就 git rebase --continue 繼續。如果覺得太複雜，隨時可以 git rebase --abort 放棄，回到 rebase 開始之前的狀態，不會有任何副作用。Rebase 後再 merge 因為是線性的，會自動 fast-forward，不需要 --no-ff。
-->
...
<!--
衝突是每個開發者遲早都會遇到的事。它不是錯誤，而是 Git 在說：「這兩個版本我沒辦法自動決定要哪個，請你來判斷。」
-->
...
<!--
衝突只在 Git 無法確定「要保留哪個版本」的時候才會發生。如果兩個人修改不同的地方，Git 通常能聰明地自動合併。衝突發生時不要慌，按照流程一步一步來，很好解決。
-->
...
<!--
衝突標記是 Git 留給你的提示，不是程式的一部分。你需要把這三個區段的標記全部刪掉，留下你想要的最終內容。可以選其中一個版本，也可以兩個都留，甚至寫一個全新的版本，完全由你決定。
-->
...
<!--
解決衝突的核心流程就三步：編輯、git add、git commit。VS Code 的 merge editor 特別好用，會用顏色區分雙方的改動，有按鈕可以直接選擇要哪個版本，不需要手動刪除標記符號。如果使用 git mergetool，可以配置外部的 diff 工具，例如 vimdiff 或 meld。
-->
...
<!--
很多人從 SVN 或其他版本控制系統轉過來，第一個驚訝的就是：Git 的分支竟然這麼快、這麼輕量。這背後有個非常優雅的設計。
-->
...
<!--
你可以打開 .git/refs/heads/ 目錄，真的去看看那些檔案，每一個分支就是一個小文字檔，裡面只有一行：那個分支最新 commit 的 SHA-1 hash。就這樣，沒有別的了。
-->
...
<!--
SVN 的 branch 是真的複製一份目錄，所以大型專案建立分支可能要等很久，占用大量空間。Git 的分支只是移動一個指標，這個設計讓「頻繁開分支」變成可能，也是現代開發流程（feature branch workflow）的基礎。理解這一點，你就不會再有「這個改動太小，不值得開分支」的心理。
-->
...
<!--
我們在切換分支的時候，Git 怎麼知道「現在在哪裡」？答案就在 .git/HEAD 這個檔案裡。
-->
...
<!--
HEAD 是 Git 的「你在哪裡」指示器。通常情況下，HEAD 指向一個分支名稱，而那個分支名稱再指向一個 commit。這樣的設計讓 Git 知道：當你提交新 commit 時，要更新哪個分支的指標。
-->
...
<!--
Detached HEAD 不是錯誤，而是一種特殊狀態。在這個狀態下你可以自由瀏覽歷史，也可以做實驗性的 commit，但如果你不先建立分支，切走之後那些 commit 就找不到了。下一個狀況題會示範如何在 detached HEAD 的情況下正確操作。
-->
...
<!--
有時候我們需要從歷史中的某個特定點開始新工作，例如從一個舊版本 hotfix，或者從某個穩定點開始做實驗。這個狀況題示範怎麼做。
-->
...
<!--
git log --oneline 把每個 commit 精簡成一行，讓你快速瀏覽歷史。找到目標 commit 後，只需要前幾位 hash（通常 7 位就夠用），不需要全部 40 位。如果有打 tag，也可以直接用 tag 名稱代替 hash，例如 git checkout -b hotfix/v1.0.1 v1.0.0。
-->
...
<!--
從特定 commit 建立分支，本質上就是在建立分支的同時指定起點。如果不指定，預設起點就是 HEAD（也就是現在的位置）。這個技巧在 hotfix 的情境特別常用：你要修的 bug 是在舊版本，所以要從舊版本的 commit 點開始，修完再 cherry-pick 或 merge 回主線。
-->
...
<!--
Hotfix 流程是從舊版本建立分支最常見的使用情境。注意第 5 步：hotfix 修完之後，可能需要把這個修正帶回到開發分支（develop），這時候用 cherry-pick 是最精準的方式，只把那個修正的 commit 搬過來，不帶任何其他的改動。Cherry-pick 是下一章的內容，這裡先知道有這個概念就好。
-->
...
<!--
第三章到這裡結束。我們從「為什麼要分支」出發，學了分支的基本操作、合併的兩種模式、常見的狀況排解，以及兩個幫助理解底層設計的冷知識。下一章我們會繼續深入 Git 的進階操作。
-->
